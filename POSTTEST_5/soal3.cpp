...
int findMaxValue(Node* root) {
    ...
    if (root == nullptr) {
        return -1; // kalau tree kosong, return -1
    }

    // Pointer untuk traversing node dalam tree
    Node* current = root;

    // 2. traversing ke kanan selama masih ada child di kanan
    while (current->right != nullptr) {
        current = current->right;
    }

    // 3. Node paling kanan pasti berisi nilai terbesar
    return current->data;
    // -----------------------------
}
