...
int findMinValue(Node* root) {
    ...
    // 1. Mengecek apakah tree kosong
    if (root == nullptr) {
        return -1; // return -1 jika tree kosong
    }

    // Buat pointer sementara untuk traversing tree
    Node* current = root;

    // 2. Selama node sementara masih punya child kiri, terus ke kiri
    // Karena di BST, nilai terkecil selalu ada di paling kiri
    while (current->left != nullptr) {
        current = current->left;
    }

    // 3. Setelah loop berhenti, 'current' menunjuk ke node paling kiri.
    return current->data;
    // -----------------------------
}
